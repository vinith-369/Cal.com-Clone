...
export default function Availability() {
    ...
    const summarizeRules = (rules) => {
        ...
        const first = dayNames[enabledDays[0]];
        const last = dayNames[enabledDays[enabledDays.length - 1]];
        const dayRange = enabledDays.length === 1 ? first : `${first} - ${last}`;
        const timeRange = `${formatTime(rules[0].start_time)} - ${formatTime(rules[0].end_time)}`;
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                        ...
                        <div>
                            <h1 style={{ fontSize: "20px", fontWeight: 600, margin: 0, display: "flex", alignItems: "center", gap: "8px" }}>
                                {editingName ? (
                                    <input
                                        ...
                                        autoFocus
                                        style={{ fontSize: "20px", fontWeight: 600, padding: "2px 8px", width: "auto", minWidth: "200px" }}
                                    />
                                ) : (
                                    <>
                            ...
                            <div style={{ fontSize: "14px", color: "var(--color-text-secondary)", marginTop: "4px" }}>
                                {summarizeRules(form.rules.filter(r => r.enabled))}
                            </div>
                        </div>
                    </div>
